...
const render = async () => {
    try {
        const all = await productosServicios.listaProductos()
        ...
        if(productos){
            productos.innerHTML = '';
            all.filter(product => product.categoria === 'StarWars').forEach(elemento => {
        ...
        if(productos2){
            ...
            all.filter(product => product.categoria === 'Consolas').forEach(elemento => {
                productos2.appendChild(nuevoProducto(elemento.name, elemento.price, elemento.imageUrl, elemento.id));
            });
        }
        if(productos3){
            productos3.innerHTML = '';
            all.filter(product => product.categoria === 'Diversos').forEach(elemento => {
